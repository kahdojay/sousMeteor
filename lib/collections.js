...
Meteor.methods({
  ...
  sendSMSInvite: function(phoneNumber, teamId, invitor) {
    ...
    if (invitee.status === STATUS.USER.NEW) {
      console.log('sendSMSInvite', invitor)
      bodyMessage = invitor.user.firstName + ' ' + invitor.user.lastName + '. invited you to Sous - ' + downloadURL;
    } else {
      var team = Teams.findOne({_id: teamId});
      bodyMessage = invitor.user.firstName + ' ' + invitor.user.lastName + ' is inviting you to ' + team.name;
    }
  ...
  deleteErrors: function(errorIdList) {
    console.log('deleteErrors called with errorIdList: ', errorIdList)
  ...
  sendOrder: function(orderId) {
    ...
    purveyor.orderEmails.split(',').forEach(function(orderEmail) {
      console.log('adding purveyor orderEmail to recipients TO array: ', orderEmail)
      recipients.push({
    ...
    team.orderEmails.split(',').forEach(function(orderEmail) {
      console.log('adding orderEmail to recipients CC array: ', orderEmail)
      recipients.push({
        email: orderEmail.trim(),
        type: 'cc'
      })
