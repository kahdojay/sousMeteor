...
Meteor.methods({
  ...
  importProducts: function(url) {
    ...
    response.data.result.forEach(function(productRow) {
      ...
      var purveyors = _.filter([productRow.purveyor1, productRow.purveyor2, productRow.purveyor3], function(purveyorCode) {
          if(purveyorCode){
            Purveyors.upsert({ purveyorCode: purveyorCode }, { purveyorCode: purveyorCode })
            purveyorId = Purveyors.findOne({ purveyorCode: purveyorCode })._id
            return purveyorId
          }
          return undefined
        }
