...
Meteor.methods({
  ...
  sendSMSCode: function(phoneNumber){
    ...
    if (user === undefined) {
      ...
      Meteor.users.update({_id: userId}, {$set: {smsTokenCount: 0, smsToken: null, smsSent: false, smsVerified: false, authToken: null}})
  ...
  loginWithSMS: function(phoneNumber, token){
    ...
    if (user.smsToken === parseInt(token.trim())) {
      ...
      Object.assign(response, {hashStampedToken})
    } else {
      response = {
        success: false
      }
