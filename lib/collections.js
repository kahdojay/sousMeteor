...
if(Meteor.isServer){
  ...
  Meteor.methods({
    ...
    sendCart: function(userId, teamId, teamOrderId) {
      ...
      if(Object.keys(teamCart.orders).length > 0){
        ...
        Object.keys(teamCart.orders).forEach(function(purveyorId){
          ...
          var orderSent = Meteor.call('sendOrder', orderId);
    ...
    registerInstallation: function(userId, deviceAttributes) {
      log.debug('registerInstallation: ', userId, deviceAttributes)

      // get the user's teamIds
      var userTeamIds = Teams.find(
        { users: { $in: [userId] } }, 
        { fields: { teamCode: 1 } }
        ).fetch()
        .map(function(team) { return team._id })

      log.debug('registering installation for team channels (ids): ', userTeamIds)

      // register installtion to channels via the user's teamIds
      Meteor.http.post(PARSE.INSTALLATION_URL, {
        ...
        body: {
          ...
          'channels': userTeamIds
        }
      })
    },
    triggerPushNotification: function(message) {
