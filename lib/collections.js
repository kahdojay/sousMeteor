...
Meteor.startup(function() {
  ...
  Mandrill.config({
    ...
    key: Meteor.settings.MANDRILL.API_KEY,  // get your Mandrill key from https://mandrillapp.com/settings/index
    // port: Meteor.settings.MANDRILL.PORT,  // defaults to 465 for SMTP over TLS
    // host: Meteor.settings.MANDRILL.LOCALHOST,  // the SMTP host
    baseUrl: 'http://localhost:61236/', // Meteor.settings.MANDRILL.BASEURL  // update this in case Mandrill changes its API endpoint URL or version
  });
});
...
Meteor.methods({
  ...
  resetImportInvite: function(phoneNumbers) {
    ...
    var firstTeam = Teams.find().fetch()[0];
