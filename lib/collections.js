...
if(Meteor.isServer){
  ...
  Meteor.methods({
    ...
    createDemoTeam: function(userId, teamId) {
      ...
      ret.import.purveyors = Meteor.call('importPurveyors', "https://sheetsu.com/apis/e36dab89", Meteor.settings.APP.DEMO_TEAMCODE, team.teamCode);
    ...
    removeUserFromTeam: function(userId, teamId){
      ...
      if(idx !== -1){
        ...
        ret.remove = Teams.update({_id: teamId}, {$set:{users: teamUsers}});
        ret.missing = false;
    ...
    updateUser: function(userId, userAttributes) {
      ...
      if (
        ...
        teamsUpdate = Teams.update({users: {$in: [userId]}},{$set:{updateAt: (new Date()).toISOString()}},{multi: true})
      }

      if (Meteor.users.findOne({_id: userId}).superUser === false){
    ...
    createMessage: function(messageAttributes, dontTriggerPushNotification) {
      // Don't trigger slack notification if it's the Sous Demo Team
        ...
        const slackAttachments = [
          {
            ...
            fields: [
              ...
              {
                title: 'Message',
                value: messageAttributes.message || 'Error: Message not found',
                short: true
              },
            ]
          }
      ...
      switch(message.type){
        case 'orderConfirmation':
          message = `${messageAttributes.purveyor} order received.`
          if(messageAttributes.message !== ''){
            message = `${message} - ${messageAttributes.author}: ${messageAttributes.message}`
          }
          break;
      }
    ...
    getTeamOrderItems: function(teamId, orderIds) {
      ...
      log.debug("Retrieving order items, with query: ", query);
