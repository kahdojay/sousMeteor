...
if(Meteor.isServer){
  ...
  Meteor.methods({
    ...
    getSettingsConfig: function() {
      log.debug('RETURNING SETTINGS CONFIG: ', Meteor.settings.APP.CONFIG)
      return Meteor.settings.APP.CONFIG;
    },
    ...
    importMessages: function(url, importTeamCode, teamCodeOverride) {
    ...
    importPurveyors: function(url, teamCode, teamCodeOverride) {
    ...
    importProducts: function(url, importTeamCode, teamCodeOverride) {
      if(undefined === importTeamCode){
        importTeamCode = 'all';
      }

      log.debug("importProducts METHOD ARGUMENTS: ", url, importTeamCode, teamCodeOverride);
    ...
    importTeamTasks: function(url, importTeamCode, teamCodeOverride) {
      if(undefined === importTeamCode){
        importTeamCode = 'all';
      }
      var ret = {
    ...
    createDemoTeam: function(userId, teamId) {
      ...
      ret.import.messages = Meteor.call('importMessages', 'https://sheetsu.com/apis/fdb1cf6b', Meteor.settings.APP.DEMO_TEAMCODE, team.teamCode);
      ret.import.teamTasks = Meteor.call('importTeamTasks', 'https://sheetsu.com/apis/15a051ab', Meteor.settings.APP.DEMO_TEAMCODE, team.teamCode);
      ret.import.purveyors = Meteor.call('importPurveyors', "https://sheetsu.com/apis/06d066f5", Meteor.settings.APP.DEMO_TEAMCODE, team.teamCode);
      ret.import.products = Meteor.call('importProducts', "https://sheetsu.com/apis/d1d0cbb3", Meteor.settings.APP.DEMO_TEAMCODE, team.teamCode);
      ...
      const slackAttachments = [
        {
          ...
          fields: [
            {
              title: 'Author',
              value: messageAttributes.author,
              short: true
            },
            {
              title: 'Message',
              value: messageAttributes.message,
              short: true
            },
          ]
        }
    ...
    updateProductCategory: function(categoryLookup, productId){
      ...
      var productIdx = categoryProducts.products.indexOf(productId);
      if(productIdx !== -1){
      ...
      ret.addProductCategory = Meteor.call('addProductCategory', categoryLookup, newProductAttributes._id);
    ...
    sendCartItems: function(userId, teamId, purveyorIds) {
      var ret = {
        success: false,
        orders: null
      }
      ...
      log.debug('\n\nSEND CART PARAMS - userId: ', userId, ' teamId: ', teamId, ' teamCode: ', team.teamCode, ' purveyorIds: ', purveyorIds, '\n\n');
      var pipeline = [
        { $match: {
            teamId: teamId,
            status: STATUS.CART_ITEM.NEW,
