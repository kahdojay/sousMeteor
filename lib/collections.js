...
Meteor.startup(function() {
  ...
  Mandrill.config({
    ...
    key: Meteor.settings.MANDRILL.API_KEY,  // get your Mandrill key from https://mandrillapp.com/settings/index
    // port: Meteor.settings.MANDRILL.PORT,  // defaults to 465 for SMTP over TLS
    host: 'localhost:3000',  // the SMTP host
...
Meteor.methods({
  createError: function(msg) {
    ...
    Errors.insert({
      userId: Meteor.users.findOne({ username: '8067892921' })._id,
      machineId: 'test',
  ...
  sendOrder: function(orderId) {
    ...
    if(purveyor.hasOwnProperty('sendEmail') === false || purveyor.sendEmail === false){
      var errMessage = {
        userId: order.userId,
        machineId: 'send-order-error:send-disabled',
        message: 'Purveyor send email order setting disabled.',
        createdAt: new Date()
      };
      Errors.insert(errMessage);
      return {
        success: false,
        error: [errMessage]
      }
    ...
    Mandrill.messages.sendTemplate({
      ...
      if(err){
        Errors.insert({
          ...
          message: 'Order Send Error - Sous has been notified, in the meantime please send order to your purveyors directly.',
          createdAt: new Date(),
        });
        // TODO: trigger slack webhook
