...
Meteor.methods({
  ...
  sendSMSInvite: function(phoneNumber, teamId, inviteeUserId) {
    ...
    if (user === undefined) {
      ...
      bodyMessage = invitee.firstName + ' ' + invitee.lastName[0] + '. has invited you to Sous - ' + downloadURL;
    } else {
  ...
  sendOrders: function(){
    ...
    if(unsentOrders.length > 0){
      ...
      unsentOrders.forEach(function(order){
        ...
        var purveyor = Purveyors.findOne({ _id: order.purveyorId }).fetch();

        // lookup BUYER info
        var team = Teams.findOne({ _id: order.teamId }).fetch();
        var user = Users.findOne({ _id: order.userId }).fetch();
        ...
        // add user info
        var contact = []
        contact.push(user.lastName)
        contact.push(', ')
        contact.push(user.firstName)
        contact.push(' • ')
        ...
        teamCityStateZip.push(' ');
        teamCityStateZip.push(team.zip);
        ...
        Object.keys(order.orderDetails.products).forEach(function(productId){
          var product = Products.findOne({ _id: productId }).fetch();
        ...
        console.log("GLOBAL MERGE VARS: ", globalMergeVars);
