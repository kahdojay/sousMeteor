...
if(Meteor.isServer){
  ...
  Meteor.methods({
    ...
    resetDemoData: function() {
      ...
      status.import.products_all = Meteor.call('importProducts', "https://sheetsu.com/apis/d1d0cbb3", Meteor.settings.APP.DEMO_TEAMCODE);
      ...
      response.data.result.forEach(function(productRow) {
        ...
        if(productAction === 'REMOVE'){
          if(productRow.hasOwnProperty('_id') === true){
            ...
            // TODO: Instead of deleting the product, just set deleted:true
    ...
    exportProducts: function() {
      ...
      allProducts.forEach(function(product) {
        ...
        var purveyorName = _.filter(allPurveyors, function(purveyor){
        ...
        var exportProductAttributes = {
          ...
          purveyor1: (purveyorName[0]) ? purveyorName[0].purveyorCode : '',
          purveyor2: (purveyorName[1]) ? purveyorName[1].purveyorCode : '',
          purveyor3: (purveyorName[2]) ? purveyorName[2].purveyorCode : '',
    ...
    removeUserFromTeams: function(userId, teamIds) {
      if(undefined === teamIds){
        teamIds = 'all';
      }
      log.debug("Removing user: ", userId, " from team(s): ", teamIds);

      if(teamIds === 'all'){
        allTeams = Teams.find({users:{$in:[userId]},notepad:{$exists:false}},{fields:{_id:1}}).fetch();
        teamIds = _.pluck(allTeams, '_id');
        console.log(teamIds)
      }
      ...
      teamIds.forEach(function(teamId){
        ...
        if(idx !== -1){
          ...
          var update = Teams.update({_id: teamId}, {$set:{users: teamUsers}});
          log.debug('UPDATE team: ', teamId, update, ' with: ', teamUsers);
        }
      })
    },
    ...
    createProduct: function(productAttributes, productLookup, cb) {
      ...
      var cb = cb || function(){}
      var productAmount = productAttributes.amount;
      if(productAmount.indexOf('/') !== -1){
        var fractionArray = productAttributes.amount.split('/')
        try {
          productAmount = parseFloat(parseInt(fractionArray[0])/parseInt(fractionArray[1]))
        } catch (e) {
      ...
      ret.product = Products.findOne(productLookup);
