...
if(Meteor.isServer){
  ...
  Meteor.methods({
    ...
    removeUserFromTeamsByTeamCode: function(phoneNumber, teamCodes) {
          ...
          Meteor.http.post(PARSE.INSTALLATION_URL, {
            ...
            if(!err && res.data.hasOwnProperty('error') === false){
              ...
              Settings.update({userId: userId}, {$set:{
                ...
                parseInstallationId: resData.installationId || null,
                parseId: resData.objectId,
                parseChannels: resData.channels,
              }})
            }
          }))
        } else {
    ...
    updateInstallation: function(userId, dataAttributes){
      ...
      if(ret.success === true){
        log.debug('updateInstallation return: ', ret);
      } else {
        log.error('updateInstallation error: ', ret);
      }
      return ret;
    },
    ...
    triggerPushNotification: function(message, teamId, userId) {
      ...
      Meteor.call('updateInstallation', user._id, {
        "badge": 0
      })
    },
