...
Meteor.methods({
  ...
  createAccount: function(phoneNumber){
    // register the account, storing the phonenumber
  ...
  sendSMSCode: function(phoneNumber){
    ...
    if( user ){
      ...
      Meteor.call('accounts-sms.sendVerificationCode', user.username, function(){
        console.log('ARGS: ', arguments);
        Meteor.users.update({_id: user.id}, {$set: {smsSent: true, smsVerified: false}});
      }.bind(this));
    }
  }.bind(this),
  // login with sms method
  loginWithSMS: function(phoneNumber, smsToken){
    console.log(Meteor.loginWithSms);
    Meteor.loginWithSms(phoneNumber, smsToken, function(){
      console.log('ARGS: ', arguments);
      Meteor.users.update({_id: user.id}, {$set: {smsSent: false, smsVerified: true}});
    }.bind(this));
