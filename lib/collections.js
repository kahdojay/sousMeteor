...
Meteor.methods({
  ...
  createAccount: function(phoneNumber){
    // register the account, storing the phonenumber, if it doesn't already exist
    // var user = Meteor.users.findOne({username: phoneNumber});
    // if (!user)
    //   Accounts.createUser({username: phoneNumber});
  },
  sendSMSCode: function(phoneNumber){
    var user = Meteor.users.findOne({username: phoneNumber});
    console.log('User FOUND:', user)
    if (user === undefined) {
      userId = Accounts.createUser({username: phoneNumber});
      Meteor.users.update({_id: userId}, {$set: {smsTokenCount: 0}})
    ...
    // TODO: limit the number of times a user can be sent an sms token to 10 before having to contact us directly
    if (user.smsTokenCount <= 10) {
      console.log('sending smsToken to ' + phoneNumber + ': ' + smsToken)
      twilio.sendSms({
        to: phoneNumber,
        from: Meteor.settings.TWILIO.FROM,
        body: 'Your Sous verification code: ' + smsToken
      }, Meteor.bindEnvironment( function(err, responseData) {
        if (!err) {
          Meteor.users.update({username: phoneNumber}, {$set: {smsToken: smsToken, smsSent: true, smsVerified: false, smsTokenCount: (user.smsTokenCount + 1)}});
      ...
      // TODO: ask them to email sous@sousapp.com because they've hit the smsToken limit
    }
  },
  loginWithSMS: function(phoneNumber, token){
    ...
    if (true) {
      ...
      Object.assign(response, Accounts._generateStampedLoginToken())
    } else {
      response = {
        success: false
      }
    ...
    return response
    // console.log('LOGGING IN WITH SMS', phoneNumber, smsToken)
  },
