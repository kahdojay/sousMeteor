...
if(Meteor.isServer){
  ...
  settingsConfig = {};
  try {
    var includeStats = fs.statSync(base + '/include.json');
    if(includeStats){
      var includeInfo = Npm.require(base + '/include.json');
  ...
  Meteor.methods({
    ...
    createDemoTeam: function(userId, teamId) {
      ...
      ret.import.products = Meteor.call('importProducts', "https://sheetsu.com/apis/a3ae1d1e", Meteor.settings.APP.DEMO_TEAMCODE, team.teamCode);
    ...
    createTeam: function(teamAttributes, userId) {
      ...
      if(team === undefined){
        ...
        if(teamAttributes.demoTeam){
          ...
          teamAttributes.teamCode = `${teamAttributes.teamCode}DEMO`
        }
      ...
      if(userSettings === undefined){
        ...
        if(userSettings.hasOwnProperty('parseId') === true && userSettings.parseId){
          ...
          if(processUpdate){
            ...
            log.debug('updateInstallation updateDataAttributes: ', updateDataAttributes)
