...
Meteor.methods({
    ...
    response.data.result.forEach(function(product) {
      ...
      if (product.category !== '') {
        ...
        if (category === undefined) {
          ...
          Categories.insert({ name: product.category })
  ...
  sanitzePhoneNumber: function(phoneNumber) {
    return phoneNumber.toString().replace(/\D/g, '');
  },

  userFindOrCreate: function(phoneNumber) {
  ...
  sendSMSInvite: function(phoneNumber, teamId, invitorUserId) {
    phoneNumber = Meteor.call('sanitzePhoneNumber', phoneNumber);
    var invitor = Meteor.users.findOne({ _id: invitorUserId });
    var downloadURL = 'beta.sousapp.com/apps';

    // find or create the user using the phone number
    var [user, created] = Meteor.call('userFindOrCreate', phoneNumber);
