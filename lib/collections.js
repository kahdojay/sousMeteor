...
Meteor.methods({
  ...
  resetImportInvite: function(phoneNumbers) {
    ...
    status.import.purveyors = Meteor.call('importPurveyors', "https://sheetsu.com/apis/06d066f5", teamId);
    status.import.products = Meteor.call('importProducts', "https://sheetsu.com/apis/d1d0cbb3", teamId);
  ...
  importPurveyors: function(url, teamId) {
    if(!teamId){
      return {
        error: 'Missing "teamId" in the parameters..'
      }
    }
    var ret = {
      'before': null,
  ...
  importProducts: function(url, teamId) {
    if(!teamId){
      return {
        error: 'Missing "teamId" in the parameters..'
      }
      ...
      if(productAction === 'REMOVE'){
        if(productRow.hasOwnProperty('_id') === true){
          ...
          if(singleProductCategory !== undefined){
            ...
            if(productIdx !== -1){
              updatedProducts = [
                updatedProducts.slice(0, productIdx),
                updatedProducts.slice(productIdx+1)
              ];
            }
        ...
        if(productRow.hasOwnProperty('_id') === true && productRow._id !== '#N/A'){
