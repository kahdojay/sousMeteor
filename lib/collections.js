let SlackAPI = Npm.require('node-slack')
let slackOrderBot = new SlackAPI(Meteor.settings.SLACK.URL)
...
Meteor.methods({
  createError: function(msg) {
    console.log('creating error: ', msg)
    ...
    twilio.sendSms({
      ...
      if (err) {
        Meteor.call('triggerError',
          'technical-error:sms',
          'We apologize for the inconvenience, but we are experiencing technical difficulties. Please try again later.',
          invitor._id
        )
      }
    }.bind(this)))
  ...
  sendSMSCode: function(phoneNumber, authToken){
    // console.log('sendSMSCode args: ', arguments)
    // Get the user by their phone number
  ...
  triggerError: function(machineKey, msg, userId) {
    console.log('TRIGGER NEW ERROR: ', machineKey, msg, ' USERID: ', userId);
    ...
    // TODO: trigger slack webhook for order errors
  ...
  sendCart: function(userId, teamId, teamOrderId) {
    ...
    console.log('TEAM CART', team.cart);
  ...
  sendOrder: function(orderId) {
    ...
    if(purveyor.hasOwnProperty('sendEmail') === false || purveyor.sendEmail === false){
      return Meteor.call('triggerError',
        ...
        'Purveyor send email order setting disabled.',
        order.userId
      )
    }
    ...
    let recipients = [
      { 
        email: purveyor.email || 'orders@sousapp.com',
        type: 'to'
      },
      { 
    ...
    team.orderEmails.split(',').forEach(function(teamContact) {
      console.log('adding teamContact to recipients array: ', teamContact)
      recipients.push({
        email: teamContact.trim(),
        type: 'cc'
      })
    })
    ...
    Mandrill.messages.sendTemplate({
      ...
      if(err){
        ...
        Meteor.call('triggerError',
          ...
          'Order Send Error - Sous has been notified, in the meantime please send order to your purveyors directly.',
          order.userId
        )

        var messageAttributes = {
            message: 'Error - Order to ' +
              Purveyors.findOne({_id: order.purveyorId}).name +
        ...
        var messageAttributes = {
            message: 'Order has been sent to - ' +
              Purveyors.findOne({_id: order.purveyorId}).name,
            author: 'Sous',
            teamId: order.teamId,
            createdAt: (new Date()).toISOString(),
